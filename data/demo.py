...
d = dataset("clevr_with_masks_train.tfrecords")

for i in tqdm(range(0,200)):
    
    images = np.zeros((5000, 3, 128, 128))
    masks = np.zeros((5000, 11, 128, 128))
    ...
    for j,raw_record in enumerate(d.take(5000)):
        ...
        mask = np.zeros((mask_orig.shape[0], 128, 128))
        for k,mask_i in enumerate(mask_orig):
            mask_i = cv.resize(mask_i, (128,128), interpolation= cv.INTER_LINEAR)
            mask[k,...] = mask_i
        masks[j,...] = mask

    hf = h5py.File(f'clever/{i}.h5', 'w')
    hf.create_dataset('images', data=images)
    hf.create_dataset('masks', data=masks)
    hf.close()

    hf = h5py.File(f'clever/{i}.h5', 'r')
    print(hf.keys())
